...
class CRF(nn.Module):
    ...
    def __init__(self, num_tags: int) -> None:
        ...
        self.start_transitions = nn.Parameter(Tensor(num_tags))
        self.end_transitions = nn.Parameter(Tensor(num_tags))
        self.transitions = nn.Parameter(Tensor(num_tags, num_tags))
    ...
    def decode(self,
               emissions: Union[Tensor, FloatTensor],
               mask: Optional[Union[Tensor, ByteTensor]] = None) -> List[List[int]]:
...
#         mask = mask.data
    ...
    def _viterbi_decode(self, emissions: FloatTensor, mask: ByteTensor) \
            -> List[List[int]]:
        # Get input sizes
        seq_length = emissions.size(0)
    ...
    def _new(self, *args, **kwargs) -> FloatTensor:
